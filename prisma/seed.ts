...
/**
 * Idempotent seed — every write is an upsert, safe to rerun via
 * `npm run db:seed` or after `npm run db:reset`. Mirrors the demo data that
 * used to live in src/core/data/{organization,branches,users,permissions}.ts
 * DEFAULTS, so the pilot demo flow behaves identically once those stores
...
async function main() {
  ...
  console.log(`Seeded tenant "${tenant.slug}" (${tenant.id}) with ${branches.length} branches, ${users.length} users, ${CAPABILITIES.length * Object.keys(ROLE_GRANTS).length} permission rows.`);
}
