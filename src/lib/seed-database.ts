...
export async function seedDatabase(prisma: PrismaClient): Promise<SeedSummary> {
  ...
  const users = [
    ...
    },
    {
      id: "u-yara",
      name: "Yara Boustani",
      email: "yara@greenhills.example",
      role: "Admin" as const,
      branchId: null,
      status: "active" as const,
      mfaEnabled: true,
      lastLoginAt: new Date(now - 1000 * 60 * 60 * 8),
      device: "MacBook Pro · Dashboard",
    },
    {
      id: "u-priya",
      name: "Priya Nazari",
      email: "priya@greenhills.example",
      role: "Designer" as const,
      branchId: "br-larnaca",
      status: "active" as const,
      mfaEnabled: false,
      lastLoginAt: new Date(now - 1000 * 60 * 60 * 30),
      device: "iPad Pro · Display Studio",
    },
    {
      id: "u-elif",
      name: "Elif Demir",
