...
/** Purely admin-entered availability (InventoryBuilder's "Units left"/"Total units") — never inferred. */
export function DisplayAvailability({ item, mode }: DisplayWidgetContext) {
  const { unitsLeft, totalUnits } = item;
  ...
  const pct = totalUnits ? Math.max(0, Math.min(100, Math.round((unitsLeft / totalUnits) * 100))) : null;
