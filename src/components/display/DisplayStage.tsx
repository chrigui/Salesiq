              ...
              >
                <DisplayProfileRenderer
                  ...
                  matchScore={(() => {
                    ...
                    return entry ? { score: entry.score, reasons: entry.reasons, narrative: narrate(entry, pack) } : null;
                  })()}
                  sessionView={view}
                  hasProposal={Boolean(proposalText)}
                />
              </motion.div>
            )}
...
/**
 * Additive sibling to CompareStage for the salesperson's own curated
 * comparison group (session.compareItemIds), reusing the exact same
 * computeCompareBadges/whyNotReasons the Companion's ComparisonExperience
...
function CompareGroupStage({
  ...
  const badgesFor = (itemId: string): string[] =>
  ...
  return (
    ...
    >
      <h2 className="mb-8 text-center text-4xl font-semibold tracking-tight">
        Comparing {group.length} properties
      </h2>
      <div className={cx("grid gap-6", group.length === 2 ? "md:grid-cols-2" : "md:grid-cols-3")}>
        {group.map((s, i) => {
          const attrs = readPropertyAttributes(pack, s.item);
          const availability = deriveAvailabilityLabel(s.item);
          const labels = badgesFor(s.item.id);
          const isWinner = s.item.id === winner.item.id;
          const whyNot = !isWinner ? whyNotReasons(pack, answers, s, winner) : [];
          ...
          return (
            ...
            >
                ...
                {labels.length > 0 && (
                  <div className="absolute right-3 top-3 z-10 flex flex-col items-end gap-1">
                    {labels.map((label) => (
                      <div
                        key={label}
                        className="flex items-center gap-1 rounded-full bg-brand px-3 py-1 text-xs font-semibold text-white"
                      >
              ...
              <div className="p-5">
                ...
                <div className="mt-3 space-y-1.5">
                  {s.item.highlights.slice(0, 3).map((h) => (
                    <div key={h} className="flex items-center gap-2 text-sm text-ink-muted">
                      <Check className="h-3.5 w-3.5 text-brand" /> {h}
                    </div>
                  ))}
                </div>
                <div className="mt-4 rounded-xl border border-white/10 bg-white/[0.03] p-3">
                  <div className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-ink-faint">
                    {isWinner ? "Why this one" : `Why it's behind ${winner.item.name}`}
                  </div>
                  <ul className="space-y-1">
                    {(isWinner ? s.reasons.slice(0, 5) : whyNot).map((r, idx) => (
                      <li key={idx} className="text-xs text-ink-faint">
                        {r}
                      </li>
                    ))}
                  </ul>
