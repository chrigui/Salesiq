...
const MAX_BYTES = 8 * 1024 * 1024; // 8MB — no object storage configured, same limit as BrochureAsset
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const assets = await prisma.displayProfileAsset.findMany({
      where: { profileId: id },
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const asset = await prisma.displayProfileAsset.create({
      data: { profileId: id, tenantId: ctx.tenantId, name, mimeType, sizeBytes: data.byteLength, data },
    });

    await logTenantAudit({
      tenantId: ctx.tenantId,
      actor: ctx.name,
      action: "display-profile.asset-uploaded",
      target: id,
